...
public class CoinCollection {
    ...
    public int removeCoins(Coin coinType, int count) {
        if (count > this.coinCount[coinType.ordinal()]) {
            count = this.coinCount[coinType.ordinal()];
    ...
    public double getTotal() {
        ...
        for (Coin c : Coin.values()) {
            total += this.getCount(c) * c.getValue();
        }
        return total;
    }
    ...
    public String toString() {
        return "NICKEL:" + getCount(Coin.NICKEL) + " " + "\nDIME:" + getCount(
                Coin.DIME) + " " + "\nQUARTER:" + getCount(
                        Coin.QUARTER) + " " + "\nTotal" + "=" + "$" + getTotal();
    }

    /**
     * Main program - designed to test out the CoinC
     */
