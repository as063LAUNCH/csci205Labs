 ...
 */
/**
 * A custom exception thrown if the cash register tried to give change before
...
public class CashRegister {
    ...
    public void collectPayment(Money moneyType, int unitCount) {
        if (unitCount < 0) {
            throw new IllegalArgumentException(String.format(
                    "Too little unitCount: $%d", unitCount));
        }
    ...
    public boolean equals(Object obj) {
        if (obj == null) {
            return false;
        } else if (((CashRegister) obj).getTransactionTotal() != this.getTransactionTotal()) {
            return false;
        } else if (((CashRegister) obj).getPaymentCollected() != this.getPaymentCollected()) {
            return false;
        } else {
            for (int i = 0; i < this.listOfItemPrices.size(); i++) {
                if (!Objects.equals(
                        ((CashRegister) obj).getListOfPurchases().get(i),
                        this.listOfItemPrices.get(i))) {
                    return false;
                }
            }
        }
